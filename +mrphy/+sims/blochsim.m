...
if doCim
  ...
  if doHist, [Mo, Mhst] = mrphy.sims.blochsim(Mi, double(Beff), T1,T2,dt,gam);
  else,      Mo = mrphy.sims.blochsim(Mi, double(Beff), T1, T2, dt, gam);
  end
...
for istep = 1:nT
  ...
  %rc_mx(istep,:) = Mx0([100, 3087, 3090]);
  %rc_my(istep,:) = My0([100, 3087, 3090]);
  %rc_mz(istep,:) = Mz0([100, 3087, 3090]);
