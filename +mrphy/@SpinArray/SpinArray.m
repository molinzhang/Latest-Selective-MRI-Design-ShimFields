...
classdef SpinArray < matlab.mixin.SetGet & matlab.mixin.Copyable
  methods (Static) % for quickly retriving attributes names. zjdsfyzs
    ...
    function n = compact(),   n={'M_', 'T1_', 'T2_', 'gam_', 'dim', 'mask', 'nM'}; end
  ...
  methods (Access = public)
    ...
    function [Mo_, Mhst_] = applypulse(obj, pulse, varargin)
      ...
      %% parsing
      [arg.loc,   arg.loc_] = deal([], []); % These are define the struc of arg.
    ...
    function beff_ = pulse2beff(obj, pulse, varargin)
      ...
      if isempty(arg.loc_), arg.loc_ = obj.extract(arg.loc); end  % if the loc is not empty, the loc_ should be compatible with loc according to the mask.
      ...
      beff_ = pulse.beff(arg.loc_, obj.mask, kw{:});
