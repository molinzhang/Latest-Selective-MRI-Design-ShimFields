...
#from mobjs import mobjs
from mrphy_rf.mobjs import SpinCube, Pulse

from adpulses_rf import metrics, penalties
...
def m2p(m2pName: str,
    ...
    *OUTPUTS*:
    ...
    cube_d = {k: f_tensor(tmp[k]) for k in ('T1', 'T2', 'M','loc')}
    ...
    #print(f_tensor(tmp['d']).shape)
    # (1, nM, xyz); (1, nM)
    
    target['d_'] = cube.extract(f_tensor(tmp['d']),None,  torch.tensor([1]))  # (1,nM,xyz)
    print(f_tensor(tmp['weight'].astype(np.float)).shape)
    ...
    pulse_d['lr'] = f_tensor(tmp['lr'])
    pulse_d['nr'] = f_tensor(tmp['nr'])
